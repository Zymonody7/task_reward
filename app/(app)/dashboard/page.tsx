...
export default function DashboardPage() {
  ...
  const handleCompleteTask = async (task: Task) => {
    ...
    if (res.success && res.data) {
      ...
      const errRes = res as { user?: { id: string; name: string; points: number } };
      if ((code === "ALREADY_COMPLETED" || code === "ALREADY_COMPLETED_TODAY") && errRes.user) {
        const today = new Date().toISOString().split("T")[0];
        const newCompletion: Completion = {
          userId: currentUser.id,
          taskId: task.id,
        ...
        const hasCompletion = completions.some(
          (c) => c.taskId === task.id && (task.type === "ONE_TIME" || c.date === today)
        );
        await mutateUser(
          {
            user: errRes.user,
            records: history,
            completions: hasCompletion ? completions : [...completions, newCompletion],
          },
          { revalidate: false }
        );
      }
      ...
      {currentUser ? (
                    ...
                    return (
                      ...
                      >
                          ...
                          <div className="flex items-center gap-2">
                            <span
                              className={cn(
                                ...
                                !isActionable ? "text-slate-500" : "text-slate-900"
                              )}
                            >
                              {task.title}
                            </span>
                        ...
                        <Button
                          ...
                          onClick={() => isActionable && handleCompleteTask(task)}
                          className={cn(
                            "min-w-[100px]",
        ...
        <div className="p-12 text-center bg-white rounded-xl border border-dashed border-slate-300">
          ...
          <p className="text-slate-500">
            Fetching your points and task data.
          </p>
        </div>
      )}
    </div>
